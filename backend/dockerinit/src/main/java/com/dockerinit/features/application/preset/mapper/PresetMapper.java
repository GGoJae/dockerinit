package com.dockerinit.features.application.preset.mapper;
...
public final class PresetMapper {
    ...
    public static PresetDetailResponse toDetail(PresetDocument doc) {
        ...
        List<PresetArtifactMetaDTO> presetArtifactMetaResponses = mapArtifactsToDTO(doc.getArtifacts());
    ...
    private static PresetArtifactMetaDTO mapArtifactToDTO(PresetArtifact a) {
        ...
        return new PresetArtifactMetaDTO(a.getFileType(), a.getFilename(), contentTypeToString, strategy,
                a.getSensitive(), normalizeEtag, a.getContentLength(), a.getStorageProvider(), a.getStorageKey());
    }

    private static List<PresetArtifactMetaDTO> mapArtifactsToDTO(List<PresetArtifact> in) {
        if (in == null || in.isEmpty()) return List.of();
        return in.stream().map(PresetMapper::mapArtifactToDTO).toList();
    }

    public static PresetArtifactResponse toArtifactResponse(PresetDocument doc) {
        List<PresetArtifactMetaDTO> presetArtifactMetaDTOS = mapArtifactsToDTO(doc.getArtifacts());
        Instant last = Optional.ofNullable(doc.getUpdatedAt())
                .orElseGet(doc::getCreatedAt);
        return new PresetArtifactResponse(presetArtifactMetaDTOS, last);
    }
